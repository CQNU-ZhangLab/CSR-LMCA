...
from .rrt.rrt import RRTEncoder
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    # init TripletAttention
    triplet_attention = TripletAttention().to(device)  # 初始化 TripletAttention 模块
    query = triplet_attention(query)
    d_k = query.size(-1)
...
class BaseCMN(AttModel):
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        # Apply RRT Encoder to att_feats
        att_feats = self.rrt(att_feats)
