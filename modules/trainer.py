...
from modules.utils import compute_grad_norm
...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # write result of test about every epoch
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        metric_value = self.mnt_best
        metric_value_str = f"{metric_value:.3f}"
        if save_best:
            # best_path = os.path.join(self.checkpoint_dir, 'model_best.pth')
            best_path = os.path.join(self.checkpoint_dir, f'model_best_{metric_value_str}.pth')
            torch.save(state, best_path)
            self.logger.info("Saving current best: model_best.pth ...")
     
    def _save_weights(self, epoch, log):
        metric_value = log[self.mnt_metric_test]
        metric_value_str = f"{metric_value:.6f}"
        if metric_value > 0.105:
            state = {
                'epoch': epoch,
                'state_dict': self.model.state_dict(),
                'optimizer': self.optimizer.state_dict(),
            ...
            weights_filename = os.path.join(self.checkpoint_dir, f'weights_epoch_{epoch}_{metric_value_str}.pth')
            torch.save(state, weights_filename)
            self.logger.info(f"Saving weights at epoch {epoch} with {self.mnt_metric_test}={metric_value_str}: {weights_filename} ...")
