...
from modules.SDAM.sdam_gradcam import GradCAM, show_cam_on_image
from modules.SDAM.sdam_model import resnet34
import pickle
import cv2
import glob
import tqdm
...
if os.path.exists("../data/sdam//thresholds.pkl"):
    with open("../data/sdam//thresholds.pkl", "rb") as f:
...
def get_model():
    ...
    model_weight_path = "..sdam/MIMIC_best_weight.pth"
...
def main(model, image_path, seg_path, mask_path, array_path):
    ...
    for target_category in list(indices):
        ...
        if mask_arr.shape != mask_arr_ass.shape:
            mask_arr = cv2.resize(mask_arr, (mask_arr_ass.shape[1], mask_arr_ass.shape[0]))
...
if __name__ == '__main__':
    ...
    image_list = glob.glob("../data/iu_xray/images/*/*.png")
    model = get_model()
    bar = tqdm.tqdm(image_list)
    for image_path in bar:
        seg_path = image_path.replace("images", "heatmap/images300_seg")

        if not os.path.exists(os.path.dirname(seg_path)):
            os.makedirs(os.path.dirname(seg_path))
        if not os.path.exists(seg_path):
            main(model, image_path, seg_path)